  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", color: "#fff", padding: "32px 20px 80px", fontFamily: "var(--font-jost), sans-serif" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        ...
        <form action={updateEvent} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, background: "#0c0c0c", border: "1px solid #1e1e1e", borderRadius: 14, padding: 22 }}>
          <input type="hidden" name="id" value={event.id} />

          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Date *</label>
            <input name="date" defaultValue={event.date} style={inputStyle} required />
          </div>
          <div>
            <label style={labelStyle}>City / Title *</label>
            <input name="city" defaultValue={event.city} style={inputStyle} required />
          </div>
          <div>
            <label style={labelStyle}>Venue *</label>
            <input name="venue" defaultValue={event.venue} style={inputStyle} required />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
          ...
          <div style={{ gridColumn: "1 / -1", display: "flex", gap: 12, marginTop: 4 }}>
            ...
            <Link href="/admin" style={{ padding: "12px 22px", borderRadius: 8, border: "1px solid #333", color: "#aaa", fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, fontSize: 13, display: "inline-flex", alignItems: "center" }}>
              Cancel
            </Link>
          </div>
        </form>
